...
export default {
   ...
   update: async (req, res) => {
      exec(res, async () => {
         const values = await Countries.findByIdAndUpdate(req.params._id, {
            name: req.body.name,
            code: req.body.code
         }, { new: true })
   ...
   delete: async (req, res) => {
      exec(res, async () => {
         const values = await Countries.findByIdAndRemove(req.params._id)
         if (values) {
   ...
   changeStatus: async (req, res) => {
      exec(res, async () => {
         if (req.body?.status) {
            const values = await Countries.findByIdAndUpdate(req.params._id, { status: req.body.status }, {new: true})
